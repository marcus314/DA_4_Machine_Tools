...
class data_Versuch_1_CMX_aircut:
    def __init__(self):
        ...
        self.trainig_datapaths = ['CMX_Alu_Tr_Air_1','CMX_Alu_Tr_Air_2','CMX_Alu_Tr_Air_3', 'CMX_Alu_Tr_Air_4','CMX_St_Tr_Air_2','CMX_St_Tr_Air_3']
...
class data_Versuch_1_I40_aircut:
    def __init__(self):
        ...
        self.trainig_datapaths = ['I40_Alu_Tr_Air_1','I40_Alu_Tr_Air_2','I40_Alu_Tr_Air_3','I40_St_Tr_Air_2','I40_St_Tr_Air_3']
        self.validation_datapaths = ['I40_St_Tr_Air_1']
        self.target_channels = ['cur_x']
        self.percentage_used = 100
...
class data_Versuch_1_CMX_prozess:
    def __init__(self):
        ...
        self.trainig_datapaths = ['CMX_Alu_Tr_Mat_1','CMX_Alu_Tr_Mat_2','CMX_Alu_Tr_Mat_3', 'CMX_Alu_Tr_Mat_4','CMX_St_Tr_Mat_2','CMX_St_Tr_Mat_3','CMX_St_Tr_Mat_4']
...
class data_Versuch_1_I40_prozess:
    def __init__(self):
        ...
        self.trainig_datapaths = ['I40_Alu_Tr_Mat_1','I40_Alu_Tr_Mat_2','I40_Alu_Tr_Mat_3','I40_St_Tr_Mat_2','I40_St_Tr_Mat_3']
        self.validation_datapaths = ['I40_St_Tr_Mat_1']
        self.target_channels = ['cur_x']
        self.percentage_used = 100
...
class data_Versuch_2_I40_aircut:
    def __init__(self):
        ...
        self.trainig_datapaths = ['I40_Alu_Tr_Air_2','I40_Alu_Tr_Air_3','I40_St_Tr_Air_1','I40_St_Tr_Air_2','I40_St_Tr_Air_3']
...
class data_Versuch_2_CMX_prozess:
    def __init__(self):
        ...
        self.trainig_datapaths = ['CMX_Alu_Tr_Mat_2','CMX_Alu_Tr_Mat_3', 'CMX_Alu_Tr_Mat_4','CMX_St_Tr_Mat_1','CMX_St_Tr_Mat_2','CMX_St_Tr_Mat_3','CMX_St_Tr_Mat_4']
...
class data_Versuch_2_I40_prozess:
    def __init__(self):
        ...
        self.trainig_datapaths = ['I40_Alu_Tr_Mat_2','I40_Alu_Tr_Mat_3','I40_St_Tr_Mat_1','I40_St_Tr_Mat_2','I40_St_Tr_Mat_3']
        self.validation_datapaths = ['I40_Alu_Tr_Mat_1']
        self.target_channels = ['cur_x']
        self.percentage_used = 100
...
class data_Versuch_4_CMX_aircut:
    def __init__(self):
        ...
        self.validation_datapaths = ['CMX_St_Val_Air_1']
        self.target_channels = ['cur_x']
        self.percentage_used = 100
...
class data_Versuch_4_I40_aircut:
    def __init__(self):
        ...
        self.validation_datapaths = ['I40_St_Val_Air_1']
        self.target_channels = ['cur_x']
        self.percentage_used = 100
...
class data_Versuch_4_I40_prozess:
    def __init__(self):
        ...
        self.validation_datapaths = ['I40_St_Val_Mat_1']
        self.target_channels = ['cur_x']
        self.percentage_used = 100
...
def calc_va(training_channels_string, train_basis_data, augmented_train_basis_data, val_basis_data):
    train_data = []
    ...
    if "v_x" in training_channels_string:
        if np.size(train_data) == 0:
            ...
            val_data = np.gradient(val_basis_data[channelcounter], dt)
            channelcounter += 1
        else:
            ...
            val_data = np.vstack((val_data,np.gradient(val_basis_data[channelcounter], dt)))
            channelcounter += 1
    ...
    if "a_x" in training_channels_string:
        ...
        if np.size(train_data) == 0:
            ...
            val_data = np.gradient(val_basis_data[channelcounter], 1)
            channelcounter += 1
        else:
            ...
            val_data = np.vstack((val_data,np.gradient(np.gradient(val_basis_data[channelcounter], dt), dt)))
            channelcounter += 1
...
def do_the_training(model, ML_params, full_train_data_ML_vector, val_data_ML_vector, full_train_target, val_target,model_string): #ONE OF THE MAIN FUNCTIONS
    if model == 'NN_Normal':
        predictions = trf.train_NN_Normal(model_string,ML_params, X_train = full_train_data_ML_vector, X_test = val_data_ML_vector, y_train = full_train_target, y_test = val_target)
    if model == 'NN_Cheap':
        predictions = trf.train_NN_Cheap(model_string,ML_params, X_train = full_train_data_ML_vector, X_test = val_data_ML_vector, y_train = full_train_target, y_test = val_target)
    if model == 'RF_Normal':
        predictions = trf.train_RF_Normal(model_string,ML_params, X_train = full_train_data_ML_vector, X_test = val_data_ML_vector, y_train = full_train_target, y_test = val_target)
    if model == 'RF_Cheap':
        predictions = trf.train_RF_Cheap(model_string,ML_params, X_train = full_train_data_ML_vector, X_test = val_data_ML_vector, y_train = full_train_target, y_test = val_target)

    if model == 'Retrain_NN_Normal':
        predictions = trf.continue_training_NN(model_string,ML_params, X_train = full_train_data_ML_vector, X_test = val_data_ML_vector, y_train = full_train_target, y_test = val_target)
    
    if model == 'NN_Cheap_formula_input':
        predictions = trf.train_NN_Cheap_formula_input(model_string,ML_params, X_train = full_train_data_ML_vector, X_test = val_data_ML_vector, y_train = full_train_target, y_test = val_target)
    return predictions
...
def transform_array(arr): #JUST FOR TESTING, MOST LIKELY SHIT --> First testing shows not shit
    if arr.shape[0] != 13:
    ...
    mrr = arr[-1]  # Last value
    if VALUE_SHIFT != 3:
        #print(f"{v_x}, {v_y}, {v_z}")
        denominator = np.sqrt(v_x ** 2 + v_y ** 2 + v_z ** 2)
        valid_indices = denominator != 0  # Vermeidung von Division durch Null
        adjusted_mrr = np.zeros_like(mrr)  # Initialisiert ein Array von Nullen gleicher Form wie mrr
        adjusted_mrr[valid_indices] = mrr[valid_indices] * (v_x[valid_indices] / denominator[valid_indices])
        mrr = adjusted_mrr
        #mrr = mrr * (v/np.sqrt(v_x ** 2 + v_y ** 2 + v_z ** 2))
    ...
    new_array = new_array = np.vstack([sgn_v_x, v_mul_a_x, v_squared_x,sgn_v_y, v_mul_a_y, v_squared_y,sgn_v_z, v_mul_a_z, v_squared_z,mrr])
...
def read_train_eval(data_params, aug_params, ML_params, augment_before_va, additional_descriptor = "", do_plot = False): 
    ...
    fullvaldata = read_fulldata(validation_datapaths[0])
    val_basis_data = fullvaldata[:-4,:]
    val_basis_data = moving_average(val_basis_data, window_size=WINDOWSIZE)
    ...
    if augmentation1 == "All_augments_by_score" and aug1arg5 == 1:
        ...
        predictions = val_target[0]
    else:
        model_string = str(model + "_"+target_channels_string[4:].capitalize()+"_"+experiment_name)
        predictions = do_the_training(model, ML_params, full_train_data_ML_vector, val_data_ML_vector, full_train_target, val_target,model_string)
    #return predictions
    
    #EVALUATE
    ###print(f"evaluating X_train {train_data_ML_vector.shape} y_train {train_target.shape} X_Test {val_data_ML_vector.shape} y_test {val_target.shape} ")
    pred_parameters1 = predictions
    true_parameters1 = val_target[0]

    #if augmentation1 == "aug_Max":
        #vergleiche wahr und predicted
...
def run_testfunctions(data_params_test, aug_params_test, ML_params_test, additional_descriptor, iterations,augment_before_va, do_save_log, do_plot = False, plot_average_predictions = False): #TODO: Save data
    ...
    with tqdm(range(iterations), desc=desc_string, ascii=True) as pbar:
        for i in pbar:
            ...
            pbar.set_postfix({"avg_integral_dev ": f" {round(this_integral_deviataon,4)} % cv: {round(this_cv,2)} %"}, refresh=True)
    ...
    #print(f"Dev % {result_deviation_percentage}, Dev std {std_deviation_percentage}")
...
#ML_params_test = RF_Normal() 
additional_descriptor = "with_formula"#"optimized_percentages" #see line 1384
